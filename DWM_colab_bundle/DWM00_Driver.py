...
def _resolve_run_inputs():
    parser = argparse.ArgumentParser(
        description='Run Data Washing Machine with a single parms file or a list of parms files.'
    )
    parser.add_argument(
        '--parms-file',
        dest='parms_file',
        default='',
        help='Path to a single parameter file (non-interactive mode).'
    )
    parser.add_argument(
        '--parms-list',
        dest='parms_list',
        default='',
        help='Path to a text file containing parameter file names, one per line (non-interactive mode).'
    )
    args = parser.parse_args()
    if args.parms_file and args.parms_list:
        parser.error('Use either --parms-file or --parms-list, not both.')

    if args.parms_file:
        return False, args.parms_file, None
    if args.parms_list:
        return True, '', open(args.parms_list, 'r')

    while True:
        choice = input('Enter 1 to run single parms file, Enter 2 to run a list of parms files ->')
        if choice == '1':
            return False, input('Enter Name of a Single Parameter File ->'), None
        if choice == '2':
            fileName = input('Enter Name of a List of Parameter Files ->')
            return True, '', open(fileName, 'r')
        print('Try again')
...
multi, parmFileName, file1 = _resolve_run_inputs()
...
while True:    
    ...
    while moreToDo:
        ...
        blockPairList = DWM42_BuildBlockPairs.buildBlockPairs(
            refDict, linkIndex, tokenFreqDict, outputFolder=iterationFolder, captureTag='05'
        )
        DWM_DataCapture.save_block_pair_list(blockPairList, os.path.join(iterationFolder, '05_blockPairList.csv'), refDict, truthDict)
        # Pair comparison views for block pairs (summary + optional token matches)
        DWM_DataCapture.save_pair_comparison_view(
            blockPairList,
            os.path.join(iterationFolder, '05_blockPairList'),
            refDict,
            tokenFreqDict,
            truthDict
        )
        ...
        if DWM10_Parms.blockCorrection and firstIteration:
            ...
            if changeCount > 0:
                tokenFreqDict=DWM16_BuildTokenFreqDict.buildTokenFreqDict(refDict)
                blockPairList = DWM42_BuildBlockPairs.buildBlockPairs(
                    refDict, linkIndex, tokenFreqDict, outputFolder=iterationFolder, captureTag='06'
                )
                ...
                # Pair comparison views for block pairs after correction
                DWM_DataCapture.save_pair_comparison_view(
                    blockPairList,
                    os.path.join(iterationFolder, '06_blockPairList_after_block_correction'),
                    refDict,
                    tokenFreqDict,
                    truthDict
                )
        ...
        if DWM10_Parms.comparator in ('TahaComparator', 'ScoringMatrixTaha'):
            DWM_DataCapture.save_taha_decision_view(
                DWM67_Tahacomparator.get_last_decisions(),
                os.path.join(iterationFolder, '07_tahaComparator'),
                refDict,
                truthDict
            )
        # Pair comparison views for linked pairs
        DWM_DataCapture.save_pair_comparison_view(
            linkedPairList,
            os.path.join(iterationFolder, '07_linkedPairList'),
            refDict,
            tokenFreqDict,
            truthDict
        )
