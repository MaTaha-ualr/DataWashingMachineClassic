...
def _get_comparator():
    ...
    if comparator == 'TahaComparator' or comparator == 'ScoringMatrixTaha':
        return 'TahaComparator', DWM67_Tahacomparator
...
def save_pair_comparison_view(pairList, filepath_prefix, refDict, tokenFreqDict, truthDict=None, trace_min_sim=0.01):
    ...
    Creates TWO CSVs:
      ...
      2) <prefix>_token_matches.csv (for ScoringMatrixKris and TahaComparator)
    ...
    with open(summary_path, 'w', newline='', encoding='utf-8') as f:
        ...
        if comparator_name == "TahaComparator":
            header.extend([
                "name_similarity",
                "first_name_similarity",
                "middle_name_similarity",
        ...
        # Optional matches (for Kris and Taha)
        write_matches = comparator_name in ("ScoringMatrixKris", "TahaComparator")
        match_rows = []
        ...
        for refID1, refID2 in sorted(pairs, key=lambda x: (x[0], x[1])):
            ...
            if comparator_name in ("Cosine", "MongeElkan"):
                ...
                if comparator_name == "TahaComparator":
                    name_details = Comp.name_similarity_details(t1_raw[:], t2_raw[:])
                    decision_band = Comp.band_label(sim, mu=mu)
            ...
            if comparator_name == "TahaComparator":
                row.extend([
                    name_details["name_similarity"],
                    name_details["first_name_similarity"],
                    name_details["middle_name_similarity"],
                    name_details["last_name_similarity"],
                    decision_band,
                ])
            if mu is not None:
                row.append(mu)
    ...
    # Write token matches if comparator supports trace
    if write_matches:
...
def save_taha_decision_view(decision_rows, filepath_prefix, refDict, truthDict=None):
    """
    Save Taha comparator decision buckets for LLM workflow.

    Outputs:
      - <prefix>_decision_summary.csv
      - <prefix>_llm_review.csv
      - <prefix>_llm_context.csv
      - <prefix>_auto_reject.csv
    """
    summary_path = f"{filepath_prefix}_decision_summary.csv"
    review_path = f"{filepath_prefix}_llm_review.csv"
    context_path = f"{filepath_prefix}_llm_context.csv"
    reject_path = f"{filepath_prefix}_auto_reject.csv"

    sorted_rows = sorted(decision_rows, key=lambda x: (x["refID1"], x["refID2"]))
    ...
    def _truth(refID1, refID2):
        if not truthDict:
            return None
        truth1 = truthDict.get(refID1)
        truth2 = truthDict.get(refID2)
        return 1 if (truth1 is not None and truth2 is not None and truth1 == truth2) else 0

    def _row(decision):
        refID1 = decision["refID1"]
        refID2 = decision["refID2"]
        row = [
            refID1,
            ", ".join(refDict.get(refID1, [])),
            ", ".join(decision.get("compared_tokens1", [])),
            refID2,
            ", ".join(refDict.get(refID2, [])),
            ", ".join(decision.get("compared_tokens2", [])),
            decision.get("similarity", 0.0),
            decision.get("name_similarity", 0.0),
            decision.get("first_name_similarity", 0.0),
            decision.get("middle_name_similarity", 0.0),
            decision.get("last_name_similarity", 0.0),
            decision.get("name_positional_similarity", 0.0),
            decision.get("address_similarity", 0.0),
            decision.get("address_number_similarity", 0.0),
            decision.get("address_number_match", False),
            decision.get("identity_evidence_score", 0.0),
            decision.get("context_evidence_score", 0.0),
            decision.get("contradiction_score", 0.0),
            decision.get("base_edge_score", 0.0),
            decision.get("local_support_score", 0.0),
            decision.get("local_conflict_score", 0.0),
            decision.get("cluster_impact", 0.0),
            decision.get("shared_neighbor_ratio", 0.0),
            decision.get("final_edge_score", 0.0),
            decision.get("edge_type", ""),
            decision.get("base_initial_decision", ""),
            decision.get("rule_decision", ""),
            decision.get("rule_reason", ""),
            decision.get("mu", 0.0),
            decision.get("reject_cutoff", 0.0),
            decision.get("review_upper", 0.0),
            decision.get("context_threshold", 0.0),
            decision.get("context_candidate", False),
            decision.get("initial_decision", ""),
            decision.get("llm_decision", ""),
            decision.get("llm_source", ""),
            decision.get("llm_reason", ""),
            decision.get("llm_confidence", ""),
            decision.get("final_decision", ""),
            decision.get("reason", ""),
            decision.get("link", False),
        ]
        if truthDict:
            row.append(_truth(refID1, refID2))
        return row
    ...
    with open(review_path, 'w', newline='', encoding='utf-8') as f:
        ...
        writer.writerow(header)
        for d in sorted_rows:
            if d.get("initial_decision", "") == "llm_review":
                writer.writerow(_row(d))
    print(f'  Data captured: {review_path}')

    with open(context_path, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(header)
        for d in sorted_rows:
            if d.get("context_candidate", False):
                writer.writerow(_row(d))
    print(f'  Data captured: {context_path}')
    ...
    with open(reject_path, 'w', newline='', encoding='utf-8') as f:
        ...
        for d in sorted_rows:
            if d.get("final_decision", "") == "reject":
                writer.writerow(_row(d))
    print(f'  Data captured: {reject_path}')
